...
personController.checkPerson = async (req, res, next) => {
  try {
    ...
    const preCheckQuery = `SELECT first_name, last_name FROM people WHERE birthday='${birthday}'`;
...
personController.addPerson = async (req, res, next) => {
  if (!res.locals.exists) {
    try {
      let data;
      let queryString;
      const { familyTree, firstName, lastName, phoneNumber, email, birthday, deathDate, streetAddress, city, state, zipCode } = req.body;
